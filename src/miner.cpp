...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("ACASHMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("ACASHMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("ACASHMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("archain-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running ACASHMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
